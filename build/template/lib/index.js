...
module.exports = function () {
   ...
   */
  var Workbook = function Workbook(data) {
    var self = this;
    self.archive = null;
    self.sharedStrings = [];
  ...
  var _get_simple = function _get_simple(obj, desc) {
    if (desc.indexOf("[") >= 0) {
   ...
   */
  var _get = function _get(obj, desc, defaultValue) {
    var arr = desc.split('.');
    try {
      while (arr.length) {
  ...
  */
  ...
  Workbook.prototype.substituteHyperlinks = function (sheetFilename, substitutions) {
    ...
    var rels = etree.parse(relsFile.asText()).getroot();
    var relationships = rels._children;
    var newRelationships = [];
    relationships.forEach(function (relationship) {
      newRelationships.push(relationship);
      if (relationship.attrib.Type === HYPERLINK_RELATIONSHIP) {
        var target = relationship.attrib.Target;
  ...
  Workbook.prototype.charToNum = function (str) {
    ...
    for (var idx = str.length - 1, iteration = 0; idx >= 0; --idx, ++iteration) {
      var thisChar = str.charCodeAt(idx) - 64,
        // A -> 1; B -> 2; ... Z->26
  ...
  Workbook.prototype.substituteArray = function (cells, cell, substitution) {
    var self = this;
    var newCellsInserted = -1,
      // we technically delete one before we start adding back
